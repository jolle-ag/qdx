...
class DeltaCodeDiscovery(environment.Environment):
    """
    ...
    def step_env(
        ...
        max_reward = jnp.max(jnp.array([0., (reward - state.current_rew) / state.init_rew - state.x_t ])) 

        delta_reward = (reward - state.current_rew) / state.init_rew
