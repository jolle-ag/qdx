...
if n_qubits_physical < 15:
    max_steps = 90 - 45 * (graph_connectivity == "All-to-All") - 26 * (graph_connectivity == "NN-2")
elif n_qubits_physical < 20:
    max_steps = 100 - 50 * (graph_connectivity == "All-to-All") - 40 * (graph_connectivity == "NN-2")
else:
    max_steps = 150 + 30 * (graph_connectivity == "NN-1") - 90 * (graph_connectivity == "All-to-All") - 30 * (graph_connectivity == "NN-2")
...
config = {
    ...
    "SEED": 1234, # Single random seed controls all experiments
    "LR": 1e-3,
    "NUM_ENVS": 16,
    "NUM_STEPS": max_steps,
    "TOTAL_TIMESTEPS": 2e6, 
...
params = None
# config["HIDDEN_DIM"] += 128*(config["GRAPH"] == "All-to-All") # Comment out

# Start with d = 3, then d=4
# This transfer learning has better convergence
for d in range(3,config["D"]+1):
    config["D"] = d
    print("Training with d:%d" % d)

    # Tweak some hyperparams
    config["TOTAL_TIMESTEPS"] += 2e6*(config["GRAPH"] == "NN-1")*(config["D"] == 3)
    config["TOTAL_TIMESTEPS"] += 4e6*(config["GRAPH"] == "NN-1")*(config["D"] == 3)*(config["N"] == 20)

    config["TOTAL_TIMESTEPS"] += 2e7*(config["D"] == 4)
    config["TOTAL_TIMESTEPS"] += 2e7*(config["GRAPH"] == "NN-1")*(config["D"] == 4)
    config["TOTAL_TIMESTEPS"] += 2e7*(config["GRAPH"] == "All-to-All")*(config["D"] == 4)


    config["MAX_STEPS"] -= 40 * (config["N"] == 20) * (config["GRAPH"] == "NN-1") * (config["D"] == 3)
    config["NUM_STEPS"] = config["MAX_STEPS"]
