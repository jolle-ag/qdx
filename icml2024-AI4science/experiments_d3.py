...
if n_qubits_physical < 11:
    max_steps = 32 - 12 * (graph_connectivity == "All-to-All")
else:
    max_steps = 40 + 24 * (graph_connectivity == "NN-1") - 16 * (graph_connectivity == "All-to-All")
...
params = None
config["TOTAL_TIMESTEPS"] += 2.5e6 * (n_qubits_physical == 9) * (config["GRAPH"] == "NN-1")
config["TOTAL_TIMESTEPS"] += 2.5e6 * (n_qubits_physical == 11)
config["TOTAL_TIMESTEPS"] += 6.5e6 * (n_qubits_physical == 11) * (config["GRAPH"] == "NN-1")
config["ENT_COEF"] += 0.01 * (n_qubits_physical == 11) * (config["GRAPH"] == "NN-1")
